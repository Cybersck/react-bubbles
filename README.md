...
- [X] What steps can you take in your web apps to keep your data secure?
    `Using https (SSL), encrypting sensitive data, making sure not to store passwords in plain text in the DB, only send data to the front end`
    `if you've already verified that user is authorized to access it, restricting access on the front end is futile if the data has already been sent`
- [X] Describe how web servers work.
    `A server that responds to http(s) requests and serves a website/webapp`
